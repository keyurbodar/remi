...
Build Remi for the Convex All Gas Hackathon: a browser-first app that turns memory concerns into repeated, reviewable, evidence-backed preparation for a medical visit.
...
Browser (profile, tasks, journal, review, consent)
...
- Firecrawl: crawls the allowlisted health sources only, extracts structured facts, stores citations and fetched times.
- AgentMail: sends reminders and reports, receives optional journal replies, records delivery status.

### Browser user journey (7 steps)

1. Sign in + context: profile for self or parent; age band, concerns, sleep, medication changes, reason for visit.
2. Five-minute check-in: word recall, orientation, browser-timed attention/reaction tasks; every response saved as it happens.
3. JEV scores live: typed domain scores, confidence, escalation gates; Convex mutations persist, reactive queries update UI.
4. Journal real incidents: structured browser form (date, person, facts, context, tags) plus the raw note stored verbatim; JEV classifies and tags; user reviews every field.
5. Trends + evidence: compare sessions and incidents; JEV computes deltas; Firecrawl citations explain official guidance.
6. Prepare the visit: editable brief with timeline, scores, trend caveats, context, citations, template-drafted questions.
7. Consent + deliver: preview first, choose recipient, explicit consent, send via AgentMail, see delivery and revoke state.

## 4. Data model (Convex) and indexes

Tables and fields:
- users / profiles / subjects: identity, relationship, ageBand, medications[], sleep, concern, createdAt
- assessments / responses: ownerId, subjectId, taskKey, promptVersion, answer, accuracy?, reactionMs?, clientTs, status
- scores: assessmentId, domain, value, confidence, modelVersion, lowConfidence, rationaleCode
- observations: source, rawText, observedAt?, factualSummary, context[], tags[], uncertainty, reviewed, corrected
- researchDocs / insights: url, publisher, extractedFacts[], fetchedAt, embedding? · subjectId, evidenceIds[], matchType, confidence
- reports / consents: subjectId, assessmentIds[], observationIds[], insightIds[], fileId?, recipient?, status · scope, grantedAt, revokedAt?
- emailThreads / messages: agentmailThreadId, direction, messageId, body, classification?, deliveryStatus, receivedAt
- workflowRuns: kind, entityId, step, attempt, state, error?, startedAt, completedAt?

Required indexes:
- responses.by_assessment_task and scores.by_assessment_domain (live results)
- assessments.by_subject_created and observations.by_subject_observed (timeline and report order)
- messages.by_agentmail_message_id (webhook idempotency)
- reports.by_owner_created and consents.by_report (delivery and revocation)
- researchDocs.by_url, plus a vector index if the JEV evidence matcher needs semantic retrieval
...
Packages: convex, @mendable/firecrawl-js, agentmail SDK, jspsych (optional, for timed tasks).
...
4. Browser journal plus optional email capture both land in the same reviewed observation model.
5. Journal preserves raw text; question drafting uses only reviewed facts via templates.
6. Firecrawl sources show publisher, URL, excerpt and fetched time; links open.
7. Convex shows queries, mutations, actions, HTTP action, workflow state, realtime UI, cron and file/report state.
...
- Copy the hackathon setup prompt into the coding agent; it installs the Convex integration, the hackathon skill and starts the build log. Run /hackathon while building so hackathon.md stays current.
- Frontend must deploy on Convex static hosting (convex.site) or ChatGPT Sites (chatgpt.site). No localhost, no other host.
- Live URL must open signed out.
- Public GitHub repo; private repos are explicitly not allowed.
- Video demo under 3 minutes attached to the submission.
- Share the app on X or LinkedIn tagging @convex, @OpenAI, @firecrawl, @agentmail.
- Submit at vibeapps.dev before Tuesday 22 September 12:00 PM PT = Wednesday 23 September 2026, 12:30 am IST.
- Winners announced September 25. Judging favors everyday apps over developer tools; the hackathon.md build log is what judges read.
- Prizes: $16,500 cash + $8,500 Codex credits across the top three plus sponsor plans. Every participant gets 20k Firecrawl credits. No OpenAI or Convex credits are provided.

Official submission checklist:
1. Register on the Luma event page.
2. Copy the hackathon setup prompt into the coding agent (installs Convex integration, hackathon skill, build log).
3. Run /hackathon while building so hackathon.md stays current: what you built, stack, live URL, demo link.
4. Public GitHub repo.
5. Frontend on convex.site or chatgpt.site.
6. Live URL opens signed out.
7. Video demo under 3 minutes attached.
...
- Submission portal: https://vibeapps.dev
- Convex static hosting docs (required frontend host): https://docs.convex.dev/hosting/static-hosting
- Convex docs: https://docs.convex.dev/

APIs:
- Firecrawl (API key + participant credits): https://firecrawl.dev

...
- **Components:** none
- **Convex features:** schema, tables, indexes, vector search, queries, mutations, actions, internal queries, internal mutations, HTTP actions, file storage, cron jobs
- **Auth:** none
- **AI models:** jev-1.13.0 (TypeSafe System One model, called from Convex actions)
- **Started:** 2026-09-19T22:13:08Z
- **Last updated:** 2026-09-20T14:31:16Z

## Log

### 2026-09-20 - r4 report and email loop (#6)
Closed the backend loop. `convex/reports/` turns reviewed state into a visit brief with no generated prose: `collectFacts` reads the newest finished check-in, its domain rollups, the reviewed observations and the matched insights with their citations; `draftQuestions` writes clinician questions from templates over those typed values only, each carrying the provenance token of the fact it came from; `renderReport` produces a styled, email-safe HTML document with the informational positioning in a banner, honest empty states, and every dynamic value escaped. `buildReport` stores the HTML as the report artifact and pins exactly the rows it rendered; `finalize` records the recipient and settles the report, and the send path reads the stored file back so the preview, the artifact and the delivery are the same bytes. `convex/email/send.ts` is the one door out: it calls the consent guard with the exact recipient and scope it is about to send, sends through AgentMail with an idempotency key derived from the report, and records the outbound message. `convex/http.ts` verifies the Svix signature on the raw body before parsing anything, writes an inbound message idempotently on `messages.by_agentmail_message_id`, schedules JEV classification of the reply, and folds delivery events back onto the outbound row. A weekly reminder cron mails from the same gate, so revoking consent stops reminders and the report together. Verified headless with real sends: 17 assertions against the isolated deployment, covering the scored check-in, the rendered brief, arrival in a real inbox with the report in the body, delivery recorded, a replayed webhook writing one row, an unsigned post refused, the reply classified `cognitive_concern`, and a revoked consent refusing the next send (`convex/reports/`, `convex/email/`, `convex/http.ts`, `convex/crons.ts`, `scripts/verify-report.ts`). Convex features: file storage, HTTP actions, cron jobs, actions, internal queries, internal mutations.

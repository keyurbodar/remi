...
// most recent row and the older rows stay as history.
...
export const requireConsent = internalQuery({
  ...
  handler: async (ctx, { reportId, recipient, scope }) => {
    ...
    if (grant.revokedAt !== undefined)
      throw new ConvexError<ConsentDenial>({ code: "consent_revoked", reportId });
    ...
    if (report?.recipient !== recipient)
      throw new ConvexError<ConsentDenial>({ code: "recipient_mismatch", reportId });
